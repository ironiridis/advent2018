...
func main() {
	...
	// find minute during which any guard was asleep most
	var guardMinuteMostMinute uint
	var guardMinuteMostDays int
	var guardMinuteMostGuard uint
	for gm := range guardMinuteCount {
		if guardMinuteCount[gm] > guardMinuteMostDays {
			guardMinuteMostMinute = gm.minute
			guardMinuteMostGuard = gm.guard
			guardMinuteMostDays = guardMinuteCount[gm]
		}
	}

	fmt.Printf("guard %d slept most often at 00:%02d (%d days)\n", guardMinuteMostGuard, guardMinuteMostMinute, guardMinuteMostDays)
	fmt.Printf("challenge response should be %d\n", guardMinuteMostGuard*guardMinuteMostMinute)
}
